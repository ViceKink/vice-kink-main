...
BEGIN
  ...
  VALUES (LEAST(user_id_a, user_id_b), GREATEST(user_id_a, user_id_b))
...
CREATE OR REPLACE FUNCTION public.get_last_message(user1 UUID, user2 UUID) RETURNS TABLE (id UUID, sender_id UUID, receiver_id UUID, content TEXT, created_at TIMESTAMPTZ, read BOOLEAN, image_url TEXT) LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.count_unread_messages(user_id UUID, other_user_id UUID) RETURNS INTEGER LANGUAGE plpgsql SECURITY DEFINER AS $$
...
CREATE OR REPLACE FUNCTION public.send_text_message(sender UUID, receiver UUID, message_content TEXT) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER AS $$
