...
RETURNS TABLE (match_id UUID, matched_at TIMESTAMPTZ, other_user_id UUID, other_user JSON) 
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN RETURN QUERY
  SELECT m.id, m.matched_at,
    CASE WHEN m.user_id_1 = user_id THEN m.user_id_2 ELSE m.user_id_1 END,
    CASE WHEN m.user_id_1 = user_id 
      THEN json_build_object('id', p2.id, 'name', p2.name, 'avatar', p2.avatar)
...
RETURNS TABLE (id UUID, sender_id UUID, receiver_id UUID, content TEXT, 
  created_at TIMESTAMPTZ, read BOOLEAN, image_url TEXT) 
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN RETURN QUERY
