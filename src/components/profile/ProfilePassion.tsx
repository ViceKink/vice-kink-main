...
const ProfilePassion = ({ passion }: ProfilePassionProps) => {
  ...
  const passionArray = Array.isArray(passion) ? passion : [passion];
    ...
    <div className="w-1/2 bg-vice-dark-purple p-4 rounded-2xl flex flex-col text-white">
      ...
      <div className="flex flex-grow items-center justify-center">
        <div className="flex flex-wrap justify-center gap-2">
          {passionArray.map((item, index) => (
            <div key={index} className="bg-white text-vice-dark-purple px-3 py-1 rounded-full text-sm">
              {item}
            </div>
          ))}
        </div>
